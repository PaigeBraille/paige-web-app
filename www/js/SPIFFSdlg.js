...
function SPIFFSnavbar() {
  ...
  var nb = 1;
  content +=
    "<td><button class='btn btn-primary'  onclick=\"SPIFFS_currentpath='/'; SPIFFSSendCommand('list','all');\">/</button></td>";
  while (nb < tlist.length - 1) {
    path += tlist[nb] + "/";
    content +=
      "<td><button class='btn btn-link' onclick=\"SPIFFS_currentpath='" +
      path +
...
function SPIFFSSendCommand(action, filename) {
  //removeIf(production)
  var response =
    '{"files":[{"name":"config.html.gz","size":"4.76 KB"},{"name":"index.html.gz","size":"21.44 KB"},{"name":"favicon.ico","size":"1.12 KB"},{"name":"config.htm","size":"19.65 KB"},{"name":"config2.htm","size":"19.98 KB"},{"name":"Testname","size":"-1"},{"name":"index2.html.gz","size":"28.89 KB"}],"path":"/","status":"Ok","total":"2.81 MB","used":"118.88 KB","occupation":"4"}';
...
function SPIFFSdispatchfilestatus(jsonresponse) {
  ...
  if (SPIFFS_currentpath != "/") {
    var pos = SPIFFS_currentpath.lastIndexOf(
      "/",
      SPIFFS_currentpath.length - 2
    );
    var previouspath = SPIFFS_currentpath.slice(0, pos + 1);
    content +=
      "<tr style='cursor:pointer;' onclick=\"SPIFFS_currentpath='" +
      previouspath +
      "'; SPIFFSSendCommand('list','all');\"><td >" +
  ...
  for (var i = 0; i < jsonresponse.files.length; i++) {
    ...
    if (String(jsonresponse.files[i].size) != "-1") {
      content += "<tr>";
      content +=
        "<td  style='vertical-align:middle; color:#5BC0DE'>" +
        get_icon_svg("file") +
        "</td>";
      content +=
        "<td  width='100%'  style='vertical-align:middle'><a href=\"" +
        jsonresponse.path +
        jsonresponse.files[i].name +
        '" target=_blank><button  class="btn btn-link no_overflow">';
      ...
      content +=
        "</td><td width='0%'  style='vertical-align:middle'><button class=\"btn btn-danger btn-xs\" style='padding: 5px 5px 0px 5px;' onclick=\"SPIFFSDelete('" +
        jsonresponse.files[i].name +
        "')\">";
  ...
  for (var i = 0; i < jsonresponse.files.length; i++) {
    if (String(jsonresponse.files[i].size) == "-1") {
      ...
      content +=
        "<td style='vertical-align:middle ; color:#5BC0DE'>" +
        get_icon_svg("folder-close") +
        "</td>";
      content +=
        "<td width='100%'  style='vertical-align:middle'><button class=\"btn btn-link\" onclick=\"SPIFFSselect_dir('" +
        jsonresponse.files[i].name +
        "');\">";
      content += jsonresponse.files[i].name;
      content += "</button></td><td>";
      content +=
        "</td><td width='0%' style='vertical-align:middle'><button class=\"btn btn-danger btn-xs\" style='padding: 5px 4px 0px 4px;' onclick=\"SPIFFSDeletedir('" +
        jsonresponse.files[i].name +
        "')\">";
      content += get_icon_svg("trash");
...
function checkSPIFFSfiles() {
  ...
  if (files.length > 0) {
    if (files.length == 1) {
      ...
      document.getElementById("SPIFFS_file_name").innerHTML = tmp.replace(
        "$n",
        files.length
      );
    }
...
function SPIFFS_UploadFile() {
  ...
  document.getElementById("uploadSPIFFSmsg").innerHTML =
    translate_text_item("Uploading ") + SPIFFS_currentfile;
  SendFileHttp(
    url,
    formData,
    SPIFFSUploadProgressDisplay,
    SPIFFSUploadsuccess,
    SPIFFSUploadfailed
  );
}

function SPIFFS_UploadText(text) {
  if (http_communication_locked) {
    alertdlg(
      translate_text_item("Busy..."),
      translate_text_item(
...
function SPIFFSUploadfailed(errorcode, response) {
  ...
  if (esp_error_code != 0) {
    ...
    document.getElementById("SPIFFS_status").innerHTML =
      translate_text_item("Error : ") + esp_error_message;
    esp_error_code = 0;
  } else {
    alertdlg(
      translate_text_item("Error"),
    ...
    document.getElementById("SPIFFS_status").innerHTML =
      translate_text_item("Upload failed : ") + errorcode;
  }
  SPIFFS_upload_ongoing = false;
  refreshSPIFFS();
}
